...
SendKinectData::SendKinectData(){
	...
	connect(sfd, result->ai_addr, (int)result->ai_addrlen);
}

int SendKinectData::sendData(D2D1_POINT_2F lelbow,
	D2D1_POINT_2F relbow,
	D2D1_POINT_2F lhand,
	D2D1_POINT_2F rhand){
	char buf[512];
	sprintf_s(buf,512, "\2%f,%f,%f,%f,%f,%f,%f,%f\3", lelbow.x, lelbow.y, relbow.x, relbow.y, lhand.x, lhand.y, rhand.x, rhand.y);
	return send(sfd, buf, 512, 0);
}
...
void CSkeletonBasics::DrawBone(const NUI_SKELETON_DATA & skel, NUI_SKELETON_POSITION_INDEX joint0, NUI_SKELETON_POSITION_INDEX joint1)
{
    ...
    if (joint0State == NUI_SKELETON_POSITION_TRACKED && joint1State == NUI_SKELETON_POSITION_TRACKED)
    {
		...
		s->sendData(m_Points[NUI_SKELETON_POSITION_ELBOW_LEFT]
			, m_Points[NUI_SKELETON_POSITION_ELBOW_RIGHT]
			, m_Points[NUI_SKELETON_POSITION_HAND_LEFT]
			, m_Points[NUI_SKELETON_POSITION_HAND_RIGHT]);
	}
    else
    {
